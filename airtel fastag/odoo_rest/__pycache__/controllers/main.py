...
class RestWebServices(Controller):
	...
	@route(['/api/<string:object_name>/<string:field_update>'], type='http', auth="none", methods=['PUT'], csrf=False)
	@__authenticate
	def updateRecordData(self, object_name, field_update, **kwargs):
		response = self._auth
		if response.get('success'):
			try:
				response.update(response['confObj']._check_permissions(object_name))
				if response.get('success') and response.get('permisssions').get('write'):
					data = self._mData
					modelObjData = request.env[object_name].sudo().search([('default_code', '=', field_update)])
					if not modelObjData:
						response['message'] = "No Record found for id(%s) in given model(%s)." % (
