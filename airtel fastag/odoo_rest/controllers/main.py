...
class RestWebServices(Controller):
	...
	def getRecordData(self, object_name, record_id, **kwargs):
		...
		# 	 'req_body': obj_name, 'params': rec_id,
		# 	})
		# api = self.['rest.api'].search['id','=',rest.id]
		# print('r2',api)
		if response.get('success'):
			try:
			# if True:
				...
				if response.get('success') and response.get('permisssions').get('read'):
					...
					if not modelObjData:
						...
						# request.env['api.log'].create(
						# 	{'api_name': api_name, 'methode': 'Get',
						# 	 'req_body': obj_name, 'params': rec_id,
						# 	 'response':response['message'] ,'status':response['success']})
					else:
		...
		request.env['api.log'].create(
			{'api_name': api_name, 'method': 'Get',
			 'req_body': obj_name, 'params': rec_id,
			 'response': response['message'], 'result': response['success']})
	...
	@route(['/api/<string:object_name>/<string:field_update>/<string:record_id>'], type='http', auth="none", methods=['POST'], csrf=False)
	@__authenticate
	def updateRecordData(self, object_name, field_update,record_id, **kwargs):
		response = self._auth
		if response.get('success'):
			try:
				response.update(response['confObj']._check_permissions(object_name))
				if response.get('success') and response.get('permisssions').get('write'):
					data = self._mData
					modelObjData = request.env[object_name].sudo().search([(field_update, '=', record_id)])
					if not modelObjData:
						response['message'] = "No Record found for id(%s) in given model(%s)." % (
						field_update, object_name)
						response['success'] = False
					else:
						_updateModelData(modelObjData, data, response.get('model_id'))
						response['success'] = True
