...
class PosOrderInherit(models.Model):
    ...
    def refund(self):
        ...
        ctx.update({'return_pos_order_restrict':True})

        print('return', self.env.context.get('return_pos_order_restrict'))
        ...
        for order in self:
            ...
            refund_order = order.with_context(ctx).copy(
                order.with_context(ctx)._prepare_refund_values(current_session)
