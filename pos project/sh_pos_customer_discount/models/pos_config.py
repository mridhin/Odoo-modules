...
class PosConfig(models.Model):
    ...
    vat_exempt_tax_config = fields.Many2one('account.tax', domain='[["tax_type","=","vat_exempt"]]', string='VAT Exempt Tax to be Used', required=True)
...
class ResPartner(models.Model):
    ...
    # @api.model
    # def default_get(self, fields):
    #     vals = super(PosOrder, self.__class__).default_get(fields)
    #     customer_discount = self.env.user.company_id.sh_customer_discount
    #     if 'sh_customer_discount' in fields:
    #         vals['sh_customer_discount'] = customer_discount
    #     return vals
...
class ResConfigSettingInherit(models.TransientModel):
    ...
    sh_customer_discount = fields.Char(string="Default Pos Discount", related='company_id.sh_customer_discount',
                                       readonly=False)
