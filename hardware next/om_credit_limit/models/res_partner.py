...
class ResPartner(models.Model):
    ...
    credit_limit_compute = fields.Monetary(
        string='Default Credit Limit ', default=-1,
    ...
    partner_credit_limit = fields.Monetary(string='Credit Limit', compute='_compute_credit_limit_compute', help='Partners Available Credit Limit.') 
    ...
    def _compute_credit_limit_compute(self):
        for partner in self:
            credit_limit_compute = self.env.company.account_default_credit_limit if partner.amount_credit_limit == -1 else partner.amount_credit_limit
            partner_credit_limit = credit_limit_compute - partner.credit 
            partner.update({
                'partner_credit_limit' : partner_credit_limit,
                'credit_limit_compute' : credit_limit_compute
            })
